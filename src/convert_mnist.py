# converter_mnist.py
# Doesn't work. May not be needed.
# Based on code in https://visualstudiomagazine.com/articles/2022/02/01/preparing-mnist-image-data-text-files.aspx
...
def main():
    ...
    print("\nCreating %d MNIST train images from binary files " \
        % n_images)
    data_path = os.path.join(cn.DATA_DIR, "MNIST")
    ...
    test_data_label = os.path.join(data_path, "t10k-labels.idx3-ubyte")
    img_file = os.path.join(cn.DATA_DIR, "mnist_train_1000.txt")
    convert(train_data_path, train_label_path, img_file, 1000)
